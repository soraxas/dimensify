...
fn spin(time: Res<Time>, mut query: Query<(&mut Transform, &Spin)>, pause: Res<Pause>) {
    ...
    for (mut transform, spin) in query.iter_mut() {
        let dist = spin.speed * time.delta_secs();
        transform.rotate_local_y(dist);
        transform.rotate_local_x(dist);
        transform.rotate_local_z(-dist);
    }
}
